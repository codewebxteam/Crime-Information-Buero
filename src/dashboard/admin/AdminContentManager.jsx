...
import { 
  ...
  Images, Users, MapPin, Calendar, Layers
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-10">
        {filtered.map(item => (
          <div key={item.id} className="bg-white dark:bg-[#111] p-4 rounded-[2rem] shadow-xl border border-transparent hover:border-red-700/10 transition-all flex flex-col relative group">
            <button onClick={() => handleDelete(item.id)} className="absolute top-4 right-4 p-2 bg-red-50 text-red-600 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-white"><Trash2 size={14} /></button>
            <div className="flex gap-4 mb-4">
              <img src={item.mediaUrl || (item.youtubeId && `https://img.youtube.com/vi/${item.youtubeId}/hqdefault.jpg`) || 'https://via.placeholder.com/150'} className="w-20 h-20 rounded-2xl object-cover shrink-0" alt="" />
            ...
            <div className="flex gap-2 mt-auto pt-3 border-t dark:border-white/5">
              {item.status === 'pending' ? (
                <>
                  <button onClick={() => updateStatus(item.id, 'approved')} className="flex-1 py-2 bg-green-600 text-white text-[9px] font-black uppercase rounded-xl flex items-center justify-center gap-2"><Check size={12} /> Approve</button>
                  <button onClick={() => updateStatus(item.id, 'rejected')} className="flex-1 py-2 bg-gray-100 dark:bg-white/5 dark:text-white text-gray-700 text-[9px] font-black uppercase rounded-xl">Reject</button>
                </>
              ) : (
                <div className="flex w-full items-center justify-between px-2">
                  ...
                  <button onClick={() => updateStatus(item.id, 'pending')} className="text-[8px] font-black uppercase text-gray-400 hover:text-red-600 underline">Reset</button>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="flex-1 bg-white dark:bg-[#0f0f0f] rounded-[3rem] p-6 md:p-8 shadow-2xl min-h-[600px] overflow-hidden border border-gray-100 dark:border-white/5">
            {activeSection === "gallery" ? <GalleryManager /> : <AnchorContentManager />}
          </div>
        </div>
      </div>

...
import {
  ...
  CheckCircle, Loader2, Award, Bell, LogOut, Calendar, Phone, Heart
} from "lucide-react";
...
const MemberDashboard = ({ initialTab = "profile" }) => {
  ...
  const [showThanks, setShowThanks] = useState(false); // Thanks message state
  ...
  const handleLogout = async () => {
    try {
      setShowThanks(true); // Show Thank You Screen
      
      // Wait for 2 seconds to show the message before actual sign out
      setTimeout(async () => {
        await signOut(auth);
      ...
      <AnimatePresence>
        {showThanks && (
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#002B5B] flex items-center justify-center text-center p-6"
          >
            <motion.div 
              initial={{ scale: 0.8, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              className="bg-white dark:bg-[#111] p-10 rounded-[3rem] shadow-2xl max-w-sm w-full"
            >
              <div className="w-20 h-20 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
                <Heart size={40} className="text-red-600 fill-red-600" />
              </div>
              <h2 className="text-3xl font-black text-[#002B5B] dark:text-white uppercase italic tracking-tighter mb-2">
                Thank <span className="text-red-700">You!</span>
              </h2>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "documents" && (
            <motion.div key="d" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 xl:grid-cols-2 gap-6 md:gap-8">
              <DocCard title="Bureau Identity Card" icon={<CreditCard size={18} />}><IdCardTemplate member={formattedDocsData} /></DocCard>
              <DocCard title="Membership Certificate" icon={<Award size={18} />}><CertificateTemplate data={formattedDocsData} /></DocCard>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
// Same sub-components as before...
const TabBtn = ({ active, onClick, label, icon }) => (
  <button onClick={onClick} className={`flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap min-w-fit flex-1 sm:flex-none ${active ? "bg-[#002B5B] text-white shadow-md scale-[1.02]" : "text-gray-400 hover:text-[#002B5B] hover:bg-gray-50 dark:hover:bg-white/5"}`}>
    {icon} {label}
  </button>
);
...
const InfoRow = ({ label, value, icon }) => (
  <div className="flex items-start gap-3">
    <div className="bg-red-50 dark:bg-red-900/20 p-2 rounded-lg text-red-700">{icon}</div>

...
import {
  User, ShieldCheck, Mail, MapPin, CreditCard, Download, Clock,
  CheckCircle, Loader2, Award, Bell, LogOut, Calendar, Phone, Heart, X
} from "lucide-react";
...
const MemberDashboard = ({ initialTab = "profile" }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-[#0a0a0a]">
      ...
      <AnimatePresence>
        {showThanks && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[#002B5B] flex items-center justify-center text-center p-6">
            <motion.div initial={{ scale: 0.8, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white dark:bg-[#111] p-10 rounded-[3rem] shadow-2xl max-w-sm w-full">
              <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <Heart size={40} className="text-red-600 fill-red-600" />
              </div>
              <h2 className="text-3xl font-black text-[#002B5B] dark:text-white uppercase italic mb-2">Thank <span className="text-red-700">You!</span></h2>
              <p className="text-gray-500 font-bold text-sm uppercase tracking-widest">For your service to the bureau.</p>
      ...
      <header className="bg-[#002B5B] text-white px-4 md:px-6 py-3 md:py-4 shadow-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <h1 className="text-base md:text-lg font-black uppercase italic tracking-tighter">Member<span className="text-red-500">HQ</span></h1>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest transition-all">
            <LogOut size={16} className="md:w-4 md:h-4" /><span className="hidden sm:inline">Logout</span>
          </button>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
        ...
        <AnimatePresence mode="wait">
          {activeTab === "profile" && (
            <motion.div key="p" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              <div className="bg-white dark:bg-[#111] p-6 md:p-8 rounded-[2rem] shadow-xl text-center border border-gray-100 dark:border-white/5 h-fit">
                <div className="w-28 h-28 md:w-32 md:h-32 mx-auto rounded-3xl border-4 border-[#002B5B] overflow-hidden mb-4 shadow-lg bg-gray-50">
                  <img src={formattedDocsData?.photo} className="w-full h-full object-cover" alt="User" />
                </div>
                <h2 className="text-lg md:text-xl font-black text-[#002B5B] dark:text-white uppercase truncate">{formattedDocsData?.name}</h2>
                <span className="text-[9px] md:text-[10px] bg-red-700 text-white px-4 py-1.5 rounded-full font-black uppercase mt-3 inline-block tracking-widest">{formattedDocsData?.designation}</span>
              ...
              <div className="lg:col-span-2 bg-white dark:bg-[#111] p-6 md:p-8 rounded-[2rem] shadow-xl border border-gray-100 dark:border-white/5">
                <h3 className="text-xs md:text-sm font-black text-[#002B5B] dark:text-white uppercase mb-6 md:mb-8 border-l-4 border-red-700 pl-4 tracking-widest">Official Record</h3>
          ...
          {activeTab === "documents" && (
            <motion.div key="d" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 xl:grid-cols-2 gap-6 md:gap-8">
              <DocCard title="Bureau Identity Card" icon={<CreditCard size={18} />}>
                <IdCardTemplate member={formattedDocsData} />
                <div className="mt-6 w-full flex justify-center">
                  <IdCardGenerator member={formattedDocsData} />
                </div>
              </DocCard>
...
const TabBtn = ({ active, onClick, label, icon }) => (
  <button onClick={onClick} className={`flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${active ? "bg-[#002B5B] text-white shadow-md scale-[1.02]" : "text-gray-400 hover:text-[#002B5B]"}`}>
    {icon} {label}
  </button>
);
...
const InfoRow = ({ label, value, icon }) => (
  <div className="flex items-start gap-3">
    <div className="bg-red-50 p-2 rounded-lg text-red-700">{icon}</div>
...
const DocCard = ({ title, icon, children }) => (
  <div className="bg-white dark:bg-[#111] p-5 md:p-6 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center w-full">
    <div className="w-full mb-5">
      <h3 className="text-xs md:text-sm font-black text-[#002B5B] dark:text-white uppercase flex items-center gap-2 tracking-widest">{icon} {title}</h3>
    </div>
    <div className="w-full overflow-x-auto no-scrollbar flex justify-center py-4 bg-gray-50 dark:bg-black/40 rounded-3xl border border-dashed border-gray-200">
      <div className="scale-[0.85] sm:scale-100 origin-center">{children}</div>
    </div>
  </div>
);

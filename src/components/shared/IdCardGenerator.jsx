...
const IdCardGenerator = ({ member }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const qrDataText = `CIB Verified Member\nName: ${member?.name || "OFFICER NAME"}\nID: ${member?.idNumber || "PENDING"}\nRank: ${member?.rank || member?.designation || "MEMBER"}`;
      ...
      pdf.setFillColor(...yellow); // Changed from lightRed to yellow as per your code
      ...
      pdf.setFontSize(15);
      pdf.text(member?.idNumber || "PENDING", 75, 300, { align: "center" });

      // Name & Details
      pdf.setTextColor(...navyBlue);
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(28);
      const nameText = (member?.name || "OFFICER NAME").toUpperCase();
      ...
      drawRow(
        "Designation",
        member?.rank || member?.designation || "MEMBER",
        detailStartY,
        true
      );
      drawRow(
      ...
      pdf.setTextColor(...yellow); // Using yellow as defined in your code
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(13);
      ...
      // SAVE 2-PAGE PDF
      // ───────────────────────────────────────────────
      pdf.save(`CIB_Official_ID_${member?.idNumber || "Member"}.pdf`);
    } catch (error) {
      console.error("Error generating PDF:", error);
      alert("Something went wrong while generating the ID Card. Please check console.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {loading ? "Generating 2-Page PDF..." : "Download Official ID"}
    </button>
  );
};

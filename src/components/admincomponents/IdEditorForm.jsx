...
const InputField = ({ label, name, value, onChange, icon }) => (
  <div className="space-y-2">
    <label className="text-[9px] font-black uppercase text-gray-400 pl-2 tracking-widest">{label}</label>
    <div className="relative group">
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full bg-gray-50 dark:bg-black border-2 border-transparent focus:border-[#001F3F] rounded-2xl py-4 pl-12 pr-4 text-xs font-bold outline-none shadow-inner transition-all"
      />
    </div>
  </div>
);
...
const IdEditorForm = ({ member, handleInput, syncToDatabase, onPreviewClick, isProcessing, isDataSynced, updateSuccess }) => {
  ...
  return (
    <div className="xl:col-span-8 bg-white dark:bg-[#111] p-8 rounded-[2rem] shadow-xl border border-gray-100 dark:border-white/5">
      <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-8 pb-4 border-b border-gray-100 dark:border-white/5 flex items-center gap-2">
      ...
      <div className="mt-12 flex flex-col sm:flex-row gap-4">
        <button
          ...
          className="flex-1 bg-[#8B0000] hover:bg-red-700 text-white p-5 rounded-2xl font-black uppercase tracking-widest text-[11px] flex items-center justify-center gap-3 shadow-xl transition-all active:scale-95 disabled:opacity-50"
        >
          {isProcessing ? <Loader2 size={18} className="animate-spin" /> : <Database size={18} />} Sync Details
        </button>

        <button
          onClick={onPreviewClick}
          disabled={!hasSyncedOnce}
          className={`flex-1 p-5 rounded-2xl font-black uppercase tracking-widest text-[11px] flex items-center justify-center gap-3 transition-all active:scale-95 ${
            hasSyncedOnce ? "bg-gray-800 text-white shadow-lg" : "bg-gray-100 text-gray-300 cursor-not-allowed"
          }`}
        >
          <Eye size={18} /> {hasSyncedOnce ? "Review Final PDF" : "Sync Required"}
        </button>
      </div>
      {updateSuccess && <div className="mt-4 text-center text-green-600 font-black text-[10px] uppercase italic">Database Updated Successfully!</div>}
    </div>
  );
};

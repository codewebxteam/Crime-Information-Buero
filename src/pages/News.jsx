  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        try {
          // Humne yahan query thodi simple rakhi hai taaki index ka lafda na ho
          const q2 = query(
    ...
    <div className="min-h-screen bg-[#f1f2f6] dark:bg-[#080808] pt-24 pb-10">
      ...
      <AnimatePresence>
        {selectedArticle && (
          ...
          >
            <motion.div
              className="bg-white dark:bg-[#111] max-w-3xl w-full rounded-2xl overflow-hidden relative shadow-2xl"
              initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              onClick={(e) => e.stopPropagation()}
            >
              <button onClick={() => setSelectedArticle(null)} className="absolute top-3 right-3 bg-black/50 text-white p-2 rounded-full z-20 hover:bg-red-700">
                <X size={20} />
              </button>
              {selectedArticle.youtubeId ? (
                ...
                <img src={selectedArticle.mediaUrl} className="w-full h-[250px] md:h-[400px] object-cover" alt="news" />
              )}
              <div className="p-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                <h2 className="text-xl md:text-2xl font-black mb-4 dark:text-white uppercase italic tracking-tight">{selectedArticle.title}</h2>
                <p className="text-sm md:text-base text-gray-600 dark:text-gray-300 leading-relaxed">{selectedArticle.description}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <div className="max-w-[1800px] mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-9 space-y-8">
            {heroArticle && (
              <div onClick={() => setSelectedArticle(heroArticle)} className="relative h-[300px] md:h-[550px] rounded-[2.5rem] overflow-hidden cursor-pointer group shadow-xl">
              ...
              {remainingFirebase.map(item => (
                <motion.div key={item.id} onClick={() => setSelectedArticle(item)} whileHover={{ y: -5 }} className="bg-white dark:bg-[#111] rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col cursor-pointer transition-all">
                  ...
                  <div className="p-7">
                    ...
                    <p className="text-[9px] text-gray-400 font-bold mt-4 uppercase tracking-tighter">{formatDate(item.createdAt)}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-4 border-t border-gray-100 dark:border-white/5">
              {remainingApi.map(item => (
                <motion.div key={item.id} onClick={() => setSelectedArticle(item)} whileHover={{ y: -5 }} className="bg-white dark:bg-[#111] rounded-[2rem] overflow-hidden shadow-sm flex flex-col cursor-pointer h-full transition-all">
                  <img src={item.mediaUrl} className="h-40 w-full object-cover opacity-90" alt="news" />
                  <div className="p-5 flex flex-col flex-1">
                    <h3 className="font-bold text-xs md:text-sm line-clamp-3 dark:text-white mt-2 flex-1 leading-snug">{item.title}</h3>
